...
{
  public override void ReportArchiveProgress(string fileName, int current, int total)
  {
    base.ReportArchiveProgress(fileName, current, total);
    EmitServiceMessage("progressMessage", $"Archiving {fileName} ({current}/{total})");
  }

  public override void ReportSummary(int archived, int skipped, int errors, bool dryRun)
  {
    base.ReportSummary(archived, skipped, errors, dryRun);
